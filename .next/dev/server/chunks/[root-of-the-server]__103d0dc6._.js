...
"[project]/src/app/api/business-config/analytics/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        // Get usage metrics
        const metrics = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].configUsageMetric.findMany({
            where,
            orderBy: {
                lastFetchedAt: 'desc'
            }
        ...
        const stats = {
            ...
            cacheHitRate: metrics.length > 0 ? metrics.filter((m)=>m.cacheHit).length / metrics.length : 0,
            targetingMatchRate: metrics.length > 0 ? metrics.filter((m)=>m.targetingMatched).length / metrics.length : 0,
            rolloutReceiveRate: metrics.length > 0 ? metrics.filter((m)=>m.rolloutReceived).length / metrics.length : 0
        };
        // Group by config key
...
//# sourceMappingURL=%5Broot-of-the-server%5D__103d0dc6._.js.map