    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Assign plans to users and manage subscriptions</p>
          <div className="mt-2 p-3 bg-blue-100 dark:bg-blue-900/20 border border-blue-300 dark:border-blue-600/50 rounded-lg">
            <p className="text-sm text-blue-700 dark:text-blue-300">
              <strong>Workflow:</strong> First{' '}
              <Link href="/admin/plans" className="underline hover:text-blue-600 dark:hover:text-blue-200 font-medium">
                manage plans
              </Link>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-800">
        {subscriptions.length === 0 && !loading ? (
          <div className="p-6 text-center">
            <p className="text-gray-500 dark:text-gray-400 mb-4">No subscriptions found in the system.</p>
            <button
              onClick={() => router.push('/admin/subscriptions/create')}
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-800">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Quotas</th>
                  ...
                  {filteredSubscriptions.map((sub) => (
                    <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm font-medium text-gray-900 dark:text-white">{sub.user.email}</div>
                          {sub.user.name && (
                            <div className="text-sm text-gray-500 dark:text-gray-400">{sub.user.name}</div>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 dark:text-white">{sub.plan.displayName}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-xs space-y-1">
                          <div className="text-gray-600 dark:text-gray-300">Projects: {sub.plan.maxProjects === null ? '∞' : sub.plan.maxProjects}</div>
                          <div className="text-gray-600 dark:text-gray-300">Devices: {sub.plan.maxDevices === null ? '∞' : sub.plan.maxDevices}</div>
                          <div className="text-gray-600 dark:text-gray-300">Mock Endpoints: {sub.plan.maxMockEndpoints === null ? '∞' : sub.plan.maxMockEndpoints}</div>
                          <div className="text-gray-600 dark:text-gray-300">API Endpoints: {sub.plan.maxApiEndpoints === null ? '∞' : sub.plan.maxApiEndpoints.toLocaleString()}</div>
                          <div className="text-gray-600 dark:text-gray-300">API Requests: {sub.plan.maxApiRequests === null ? '∞' : sub.plan.maxApiRequests.toLocaleString()}</div>
                          <div className="text-gray-600 dark:text-gray-300">Logs: {sub.plan.maxLogs === null ? '∞' : sub.plan.maxLogs.toLocaleString()}</div>
                          <div className="text-gray-600 dark:text-gray-300">Sessions: {sub.plan.maxSessions === null ? '∞' : sub.plan.maxSessions.toLocaleString()}</div>
                          <div className="text-gray-600 dark:text-gray-300">Crashes: {sub.plan.maxCrashes === null ? '∞' : sub.plan.maxCrashes}</div>
                          <div className="text-gray-600 dark:text-gray-300">Business Config Keys: {sub.plan.maxBusinessConfigKeys === null ? '∞' : sub.plan.maxBusinessConfigKeys}</div>
                          <div className="text-gray-600 dark:text-gray-300">Localization Languages: {sub.plan.maxLocalizationLanguages === null ? '∞' : sub.plan.maxLocalizationLanguages}</div>
                          <div className="text-gray-600 dark:text-gray-300">Localization Keys: {sub.plan.maxLocalizationKeys === null ? '∞' : sub.plan.maxLocalizationKeys.toLocaleString()}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
