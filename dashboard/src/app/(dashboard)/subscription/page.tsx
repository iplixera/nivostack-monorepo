...
export default function SubscriptionPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Subscription & Account</h1>
        <p className="text-gray-500 dark:text-gray-400">Manage your subscription, view usage, and upgrade your plan</p>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="text-2xl font-bold text-gray-900 dark:text-white">{subscription.plan.displayName}</div>
                {subscription.plan.price === 0 && (
                  <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">Free Forever - Monthly Renewal</div>
                )}
                {subscription.plan.price > 0 && (
              ...
              {subscription.plan.price === 0 && (
                <>
                  <div>
                    <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">Period Started</div>
                    <div className="text-gray-900 dark:text-white">
                      {new Date(subscription.currentPeriodStart).toLocaleDateString()}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">Renews On</div>
                    <div className="text-gray-900 dark:text-white">
              ...
              {subscription.plan.price > 0 && (
                <>
                  <div>
                    <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">Current Period Start</div>
                    <div className="text-gray-900 dark:text-white">
                      {new Date(subscription.currentPeriodStart).toLocaleDateString()}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">Current Period End</div>
                    <div className="text-gray-900 dark:text-white">
                      {new Date(subscription.currentPeriodEnd).toLocaleDateString()}
                    </div>
                  </div>
                  <div className="md:col-span-2">
                    <Link
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-900 dark:text-white rounded-lg transition-colors text-sm border border-gray-200 dark:border-gray-700"
                    >
          ...
          {usage && (
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Usage Summary</h2>
      ...
      {activeTab === 'usage' && usage && (
        <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Detailed Usage Statistics</h2>
            ...
            {subscription.plan.price === 0 && (
              <div className="mb-4 p-4 bg-blue-900/20 border border-blue-600 rounded">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-gray-900 dark:text-white font-semibold">{usage.daysRemaining} days until renewal</span>
                </div>
      ...
      {activeTab === 'plans' && (
        <div className="space-y-6">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Upgrade Your Plan</h2>
            <p className="text-gray-500 dark:text-gray-400">Choose a plan that fits your needs. Payment integration coming soon.</p>
          </div>

          {paidPlans.length === 0 ? (
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
              <p className="text-gray-500 dark:text-gray-400 text-center">No upgrade plans available at this time. Please check back later.</p>
            </div>
          ) : (
      ...
      {activeTab === 'history' && (
        <div className="space-y-6">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Subscription History</h2>
            <p className="text-gray-500 dark:text-gray-400">View your billing period history and usage statistics</p>
          </div>

          {history.length === 0 ? (
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
              <p className="text-gray-500 dark:text-gray-400 text-center">No subscription history yet. History will appear here after your first billing period.</p>
            </div>
          ) : (
            <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-800">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-300 uppercase tracking-wider">Period</th>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-300 uppercase tracking-wider">Amount</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-300 uppercase tracking-wider">Usage</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                  {history.map((period) => (
                    <tr key={period.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-600 dark:text-gray-300">{period.plan?.displayName || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 dark:text-white">
                          ...
                          {period.totalPaid !== null && period.totalPaid !== period.totalInvoiced && (
                            <span className="text-xs text-gray-500 dark:text-gray-400 ml-1">(Paid: ${period.totalPaid.toFixed(2)})</span>
                          )}
                        </div>
                      </td>
      ...
      {activeTab === 'profile' && (
        <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Account Type</label>
...
function UsageCard({ label, used, limit }: { label: string; used: number; limit: number | null }) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
      ...
      <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{displayUsed}</div>
...
function PlanCard({ plan, currentPlanName, token, onUpgradeSuccess }: { plan: Plan; currentPlanName: string; token: string | null; onUpgradeSuccess: () => void }) {
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 rounded-lg border-2 p-6 relative ${
      isPopular ? 'border-blue-600' : 'border-gray-200 dark:border-gray-800'
    }`}>
      {isPopular && (
      ...
      <div className="mb-4">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1">{plan.displayName}</h3>
      ...
      <button
        ...
        className={`w-full py-2 px-4 rounded font-medium transition-colors ${
          isCurrentPlan
            ...
            : upgrading
            ? 'bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-300 cursor-wait'
            : isPopular
