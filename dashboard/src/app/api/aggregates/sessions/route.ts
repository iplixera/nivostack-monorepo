...
export async function GET(request: NextRequest) {
  try {
    ...
    // Dimension filters
    const entryScreen = searchParams.get('entryScreen')
    const exitScreen = searchParams.get('exitScreen')
    ...
    if (entryScreen) where.entryScreen = entryScreen
    if (exitScreen) where.exitScreen = exitScreen
    ...
    const [aggregates, total] = await Promise.all([
      prisma.sessionAggregate.findMany({
        where,
        orderBy: { period: 'desc' },
        take: limit,
        skip: offset,
      }),
      prisma.sessionAggregate.count({ where }),
    ])
    ...
    console.error('Get aggregate sessions error:', error)
