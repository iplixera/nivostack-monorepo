...
async function testExceedApiRequests() {
  console.log('\n=== Testing API Requests Quota Exceeded ===\n')
  ...
  // Calculate how many requests we need to send to exceed limit
  const remainingRequests = apiRequests.limit ? Math.max(0, apiRequests.limit - apiRequests.used) : 1000
  const requestsToSend = remainingRequests + 5 // Send 5 more than limit
  
  console.log(`Sending ${requestsToSend} requests (${remainingRequests} to reach limit + 5 to exceed)...\n`)
  ...
  for (let i = 0; i < requestsToSend; i++) {
    ...
    if (traceResponse.ok) {
      successCount++
      if (i < 3 || (i >= remainingRequests - 2 && i <= remainingRequests + 2)) {
...
testExceedApiRequests().catch(console.error)
